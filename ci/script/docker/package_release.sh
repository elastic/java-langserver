...
if [ $# -lt 1 ]; then
  echo "java langserver's version missing"
  exit 2
fi
...
docker run \
    ...
    /bin/bash -c "mvn -Dtycho.mode=maven -DnewVersion=$VERSION -D-Pserver-distro org.eclipse.tycho:tycho-versions-plugin:set-version && \
